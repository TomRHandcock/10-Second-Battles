...
function agentManager.move(dt)																									--This procedure is responsible for moving the agent the player controls. It is called from agentmanager.update()
	if agentManager.selectedAgent ~= 0 then																				--These two lines will loop through every agent if there are agents currently spawned.
		if agent_dead[agentManager.selectedAgent] == -1 then
			tile1 = map[math.floor((((agent_collider_x1[agentManager.selectedAgent] + agent_collider_x2[agentManager.selectedAgent]) / 2)-map_offsetX)/(tile_width*map_scaleX))+1][math.floor((agent_collider_y1[agentManager.selectedAgent]-map_offsetY)/(tile_height*map_scaleY))+1]		--These four lines will determine the values of the four tiles immediately adjacent to the player.
			tile2 = map[math.floor((agent_collider_x2[agentManager.selectedAgent]-map_offsetX)/(tile_width*map_scaleX))+1][math.floor((((agent_collider_y1[agentManager.selectedAgent] + agent_collider_y2[agentManager.selectedAgent]) / 2) -map_offsetY)/(tile_height*map_scaleY))+1]
			tile3 = map[math.floor((((agent_collider_x1[agentManager.selectedAgent] + agent_collider_x2[agentManager.selectedAgent]) / 2)-map_offsetX)/(tile_width*map_scaleX))+1][math.floor((agent_collider_y2[agentManager.selectedAgent]-map_offsetY)/(tile_height*map_scaleY))+1]
			tile4 = map[math.floor((agent_collider_x1[agentManager.selectedAgent]-map_offsetX)/(tile_width*map_scaleX))+1][math.floor((((agent_collider_y1[agentManager.selectedAgent] + agent_collider_y2[agentManager.selectedAgent]) / 2)-map_offsetY)/(tile_height*map_scaleY))+1]
